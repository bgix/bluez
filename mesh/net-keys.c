...
uint32_t net_key_add(const uint8_t master[16])
{
	...
	key->ref_cnt++;
	result = mesh_crypto_k2(master, p, sizeof(p), &key->nid, key->enc_key,
								key->prv_key);
	if (!result)
		goto fail;

	result = mesh_crypto_k3(master, key->net_id);
	if (!result)
		goto fail;

	result = mesh_crypto_nkbk(master, key->snb_key);
	if (!result)
		goto fail;

	result = mesh_crypto_nkpk(master, key->pvt_key);
	if (!result)
		goto fail;
